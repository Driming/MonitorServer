...
public class WebSocketSendService{
	 ...
	 * 发送任务配置更改
 	 */
	/*public void updateTaskConfigure(CollectionTask ct){
		updateTaskConfigure(ct.getCsid(), ct.getCtid(), ct.getConfig());
	}

	public void updateTaskConfigure(String csid, String ctid, String config) {
	...
	//根据最新的collection_task字段进行更改，将各个字段逐个放进JSON对象里面并返回ct对象
	public void setTaskConfigure(CollectionTask ct){
		JSONObject json = new JSONObject();
		json.put("id", ct.getCtid());
		json.put("name", ct.getName());
		json.put("config", ct.getConfig());
		json.put("driverName", ct.getDriverName());
		json.put("driverVersion", ct.getDriverVersion());
		json.put("type", ct.getType());
		json.put("dispatchConfig", ct.getDispatchConfig());
		template.convertAndSend("/user/" + ct.getCsid() + "/set_task", json.toString());
	}
